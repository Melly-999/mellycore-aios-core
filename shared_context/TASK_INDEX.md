...
| `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001` | M3 | `ELIGIBLE` — awaiting Operator authorization | **Bounded: the primary Command Center cockpit surface only** (`site/dashboard.html` + its dedicated `site/css/dashboard.css`, `site/js/dashboard.js`, `site/data/*.json`). Implements `docs/specs/MELLYCORE_COMMAND_CENTER_COCKPIT_SPEC_001.md` on the existing static HTML/CSS/vanilla-JS architecture. Owns exactly nine surfaces: Command Bar, Context Management, Model Routing, Knowledge & Operations Graph, AI Agents / Loop Registry, Architecture Snapshot, Attention Queue, AI Operations Workflow, Primary Navigation. **Does not own the other M3 surfaces, any secondary destination page, `site/index.html`, or any homepage section.** No framework migration, backend, provider, runtime activation, live ingestion, dependency addition without justification, or fake-live telemetry. Dependencies satisfied (spec owner, design-system delta, direction decision, safety vocabulary). Being `ELIGIBLE` does not authorize start, merge, or deployment. |
| `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-ACCEPTANCE-001` | M3 | `PLANNED`, `BLOCKED` on `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001` | Independent acceptance of the cockpit implementation: six-width responsive validation, accessibility, reduced motion, truthfulness audit, and V3.1 fidelity comparison. Must be run by a separate agent/session from the implementation. Task ID newly minted here. |
