...
touches `site/**`. None of these mints a task identifier or authorizes work.
This section does not reorder the Global Pointer or another independent lane.

The former "optional Homepage Spec / M3 planning" recommendation is now
partly materialized: `MELLYCORE-COCKPIT-V3-CANONICALIZATION-001` established
a canonical cockpit spec owner and minted
`MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001` (`ELIGIBLE`) in the Cinematic AIOS
section below. That mint authorizes nothing by itself and does not reorder
this section.
...
| `MELLYCORE-CLAUDE-DESIGN-HANDOFF-REVIEW-001` | M1 | `SUPERSEDED` — premise corrected | **This row's original description was factually wrong (finding `CV3-CANON-001`, P1).** It described `.agents/`, `.claude/skills/`, and `skills-lock.json` on `design/mellycore-claude-design-sync-001` as "the externally generated Claude Design System handoff (tokens, components, site/cockpit UI kits, `SKILL.md`)". A fresh read-only snapshot at `55bb5e9` found those paths contain only eight **third-party Higgsfield AI** media/marketing skill packages (`higgsfield-brandkit`, `-generate`, `-marketplace-cards`, `-product-photoshoot`, `-soul-id`, `-video-explainer`, `-websites`, `-youtube-thumbnail`), with `.claude/skills/` holding only symlinks to them. No MellyCore design tokens, components, UI kits, or cockpit `SKILL.md` exist there; those packages are **REJECTED**, not adopted. The genuine V3.1 design input is unversioned and external, and was reviewed directly by `MELLYCORE-COCKPIT-V3-CANONICALIZATION-001`. Evidence: `docs/tasks/MELLYCORE-COCKPIT-V3-CANONICALIZATION-001.md` §3–§4. |
| `MELLYCORE-HERO-DIRECTION-DECISION-001` | M1 | `RESOLVED` — no bake-off required | Resolved by surface separation rather than by choosing a single winner. **Homepage/hero** retains Source Arena (direction A); the **Command Center cockpit** adopts MellyCore Cockpit V3.1. The two surfaces are complementary and separately owned, so directions A–D no longer compete for one slot. Operator-directed; recorded in `shared_context/DESIGN_SYSTEM.md` §"Surface Ownership" and `docs/tasks/MELLYCORE-COCKPIT-V3-CANONICALIZATION-001.md` §5. Not a claim that either surface is implemented. |
| `MELLYCORE-DESIGN-SYSTEM-CINEMATIC-AMENDMENT-001` | M1 | `COMPLETE` (cockpit scope only) — `PASS_WITH_LIMITATIONS` | Minimal surface-scoped amendment applied to `shared_context/DESIGN_SYSTEM.md` by `MELLYCORE-COCKPIT-V3-CANONICALIZATION-001`: added "Surface Ownership" and "Command Center Cockpit" sections, and scoped the homepage lead-image constraint to its own surface. No global rule deleted. **Limitation:** covers only the cockpit delta needed to authorize implementation; a broader cinematic amendment for hero/showcase surfaces remains unperformed and may be reopened under a new identifier if required. |
| `MELLYCORE-COCKPIT-V3-CANONICALIZATION-001` | M1→M3 bridge | `COMPLETE` — `PASS_WITH_LIMITATIONS` | Reconciled the Operator-accepted MellyCore Cockpit V3.1 direction with canonical state; corrected `CV3-CANON-001`; established `docs/specs/MELLYCORE_COMMAND_CENTER_COCKPIT_SPEC_001.md` as the canonical owner of `site/dashboard.html`; minted the bounded implementation task below. Docs/state only; no `site/**` change. Evidence: `docs/tasks/MELLYCORE-COCKPIT-V3-CANONICALIZATION-001.md`. |
...
| `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001` | M3 | `COMPLETE_LOCAL_UNACCEPTED` — `PASS_WITH_LIMITATIONS` | Implemented the nine owned cockpit surfaces on the existing static HTML/CSS/vanilla-JS architecture: Command Bar, Context Management, Model Routing, deterministic 45-node/66-edge Knowledge & Operations Graph, AI Agents / Loop Registry, Architecture Snapshot, Attention Queue, AI Operations Workflow, and Primary Navigation. Six-width Chrome self-validation, keyboard interaction, reduced motion, truthfulness, 696 tests, and the project validator passed. The retained legacy dashboard tab shell is a bounded V3.1 fidelity deviation. **Not independently accepted, pushed, merged, or deployed.** Evidence: `docs/tasks/MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001.md`. |
| `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-ACCEPTANCE-001` | M3 | `REMEDIATION_REQUIRED` — superseded | Independent acceptance of `86f496e...` found two P1 findings: material graph-label collisions/inspector occlusion, and mobile right-column panels (AI Agents, Architecture Snapshot, Attention Queue) collapsing to ~2px at 390×844. Not merge/deployment authorization. Superseded by remediation + reacceptance below. |
| `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-REMEDIATION-001` | M3 | `COMPLETE` — `VALIDATED_LOCAL_REMEDIATION` | Fixed both P1 findings: deterministic collision-scoring graph-label placement with leader lines and enlarged essential typography; intrinsic-height mobile fix for the right-column panels. Commit `04208809c80655d65710bbc06266de7cd157f8ff` on `fix/mellycore-cockpit-v3-implementation-remediation-001`, one commit above `86f496e...`. Touched only `site/css/dashboard.css`, `site/js/dashboard.js`, `shared_context/AGENT_HANDOFF.md`. Not pushed, merged, or deployed. |
| `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-REACCEPTANCE-001` | M3 | `COMPLETE` — `PASS` | Independent re-acceptance of `04208809c8...157f8ff` from a fresh read-only worktree at the exact SHA. Both P1 findings independently confirmed fixed across all six required viewports in default and node-selected states (zero label overlaps/occlusions/stage escapes; exact mobile panel dimensions; no horizontal overflow). Graph provenance, 696 tests, project-state validator, `node --check`, and `git diff --check` all re-passed. `ACCEPTED_FOR_INTEGRATION: YES`, `ACCEPTED_FOR_VERCEL_PORTFOLIO_RELEASE: YES`, `PORTFOLIO_FRONTEND_READY: YES`. No `site/**` change. Evidence: `docs/tasks/MELLYCORE-COCKPIT-V3-IMPLEMENTATION-REACCEPTANCE-001.md`. |
| `MELLYCORE-COCKPIT-V3-INTEGRATION-PORTFOLIO-RELEASE-001` | M3 | `ELIGIBLE` — not yet executed | Next canonical cockpit task after reacceptance `PASS`. No push, merge, or deployment authorized by this row. |
| M3 remaining Flagship Command Center product-surface tasks | M3 | `PLANNED` | The remaining fifteen product/UI projection surfaces (Mission Control/Overview, Runtime Constellation, Runs, Providers, Tools/MCP, Shared Context, Memory, Artifacts, Cost/Usage, Observability, Governance/Approvals, Hardware/Local AI, and the standalone Agents/Models/Context destinations). No task IDs minted; each requires its own spec/review pass. The Knowledge & Operations Graph, Context Management, and Model Routing panels **inside the primary cockpit** are carved out to `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001` above; their standalone full surfaces remain planned here. |
...
- **DESIGN** — the former chain `MELLYCORE-CLAUDE-DESIGN-HANDOFF-REVIEW-001` → `MELLYCORE-HERO-DIRECTION-DECISION-001` → `MELLYCORE-DESIGN-SYSTEM-CINEMATIC-AMENDMENT-001` is closed for cockpit purposes by `MELLYCORE-COCKPIT-V3-CANONICALIZATION-001` (see the M1 rows above for each outcome). Remaining design scope is the optional broader hero/showcase cinematic amendment, which owns `shared_context/DESIGN_SYSTEM.md` and does not block cockpit implementation.
- **COCKPIT** — `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-001` → `MELLYCORE-COCKPIT-V3-IMPLEMENTATION-ACCEPTANCE-001`. Owns `site/dashboard.html` and its dedicated assets, plus `docs/specs/MELLYCORE_COMMAND_CENTER_COCKPIT_SPEC_001.md`. Disjoint from the homepage owner (`site/index.html`, `MELLYCORE_HOMEPAGE_SPEC_001.md`), so it may run concurrently with the lanes above.
