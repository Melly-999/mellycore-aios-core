...
39. `MELLYCORE-LOOP-REPORT-ONLY-DRY-RUN-001` — immediate next recommended task: hand-run the `project-health` loop once, produce a real run ledger, and check the deterministic guard against real input. One loop, one run, report-only; no new capability, no scheduler, no write scope, no push.
