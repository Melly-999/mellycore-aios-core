...
from tests.mellycore_batch_fixtures import FakeBatchProvider, make_manifest_dict, no_network
...
class FakeProviderLifecycleTests(unittest.TestCase):
    ...
    def test_full_lifecycle_upload_create_poll_download(self) -> None:
        with no_network():
            file_id = self.provider.upload_input_file(Path(self.input_summary.path), purpose="batch")
            ...
            self.provider.set_batch_status(
                snapshot.batch_id, status="completed", output_file_id="out-1", completed_at="2026-01-01T00:00:00Z"
            )
            ...
            downloaded = self.provider.download_file("out-1", self.root / "downloaded.jsonl")
    ...
    def test_lifecycle_cancel_path(self) -> None:
        with no_network():
            file_id = self.provider.upload_input_file(Path(self.input_summary.path), purpose="batch")
            snapshot = self.provider.create_batch(
                input_file_id=file_id, endpoint="/v1/responses", completion_window="24h", metadata={}
            )
